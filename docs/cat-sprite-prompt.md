...
# 2枚目：ひとりでいるときのポーズ（`images/cat-*-relax.png`）

お世話をしていないときに使う8ポーズ。**柄ごとに1枚必要**で、作り直すときは
1枚目も含めて全シートを `tools/build_cat_sheet.py` に一緒に渡す（共通の縮尺を取り直すため）。

毛色の行を柄ごとに差し替える:

- きじとら … 茶色と黒の縞模様、口のまわり・胸・足先は白、目は黄緑
- ちゃしろ … クリームホワイトの地にオレンジ茶色の模様、目は黒目がち
- くろねこ … 全身が黒、目は山吹色
- はちわれ … 黒と白、顔の中央から胸・足先が白、目は黄緑

```
同じ1匹の猫のキャラクターを、3×3のグリッドに並べた1枚のスプライトシートを作ってください。
既存のシートと同じ画風・同じ大きさで、ポーズだけ違うものです。

【キャラクター設定】
・かわいい四足の子猫、デフォルメ2頭身、丸くて大きな顔
・毛色：茶色と黒の縞模様（キジトラ）、口のまわり・胸・足先は白
・輪郭線：こい茶色の太い線。目は大きく黄緑色
・9マスすべて完全に同じ猫（毛色・模様・体型・線の太さを統一）

【画風】
・フラットなベタ塗りのアニメ・絵本調、影やグラデーションは最小限
・高コントラストでシンプル、かわいらしいマスコット的なデザイン

【9マスのポーズ】（左上から右方向の順）
上段左：仰向けに寝てお腹を見せ、前足を曲げて「かまって」とねだっている
上段中央：仰向けでゴロゴロと体をひねり、しっぽを振っている
上段右：座って前足を上げ、顔をなめて毛づくろいしている
中段左：しっぽを追いかけてくるりと回っている
中段中央：香箱座り（前足をたたんで丸く座る）で満足そうに目を細めている
中段右：横向きに寝そべって、気持ちよさそうに伸びている
下段左：びっくりして毛が逆立ち、目を丸くしている
下段中央：おすわりで首を伸ばし、上を見上げている
下段右：ふてくされて後ろを向き、しっぽだけこちらに向けている
...
・グリッドの枠線・区切り線・番号・文字・ロゴは一切描かない
・各マスの中央に猫を配置し、9匹すべて同じ大きさ（体の高さを揃える）
・マスの枠に触れないよう、周囲に10%程度の余白を空ける
・となりのマスにはみ出さない、影を落とさない。小物は描かない
...
- **上段左は柄によって絵が変わりやすい**（仰向けを描く生成AIと、座り姿を描く生成AIがある）。
  ゲームでは上段左を使わず、仰向けは上段中央のマスを使っている
- 毛色がずれた生成結果は採用しない（お世話中と待機中で猫の色が変わってしまう）。
  平均の明るさと白抜きの割合を1枚目と比べて確認するとよい
- シートごとに絵の大きさがぶれることがある。`tools/build_cat_sheet.py` が
  セルの高さの中央値をシート間でそろえてから共通の縮尺をかけるので、
  1.1倍くらいのぶれは自動で吸収される
